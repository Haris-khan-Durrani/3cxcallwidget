    ...
    <div class="rp-page">
      ...
      <div class="rp-header">
        <div class="rp-header-left">
          ...
          <div class="rp-header-left">
            <h1 class="rp-title">Dialer Reports</h1>
            <p class="rp-sub">Track every lead and call initiated through your dialers</p>
          </div>
        </div>
      ...
      <div class="rp-filter-bar">
        <!-- Dialer selector removed for embed -->
          ...
          <div class="rp-filter-item">
            ...
            <input type="text" v-model="agentFilter" class="rp-select" placeholder="e.g. 750" />
          </div>
...
function playInline(r) {
  ...
  activeAudioUrl.value = `${apiBase}/api/admin/dialer-widgets/${r.dialerId}/recordings/${r.recording_id}/listen?token=${encodeURIComponent(token)}`
  nextTick(() => {
    if (audioRef.value) {
      audioRef.value.load()
...
const currentDialer = computed(() => report.value?.dialer || null)
...
const filteredRecords = computed(() => {
  ...
  if (agentFilter.value.trim()) {
    ...
    r = r.filter(x => (x.agent_extension||'').toLowerCase().includes(t))
  }
...
const agentPerformanceStats = computed(() => {
  if (!currentDialer.value || !report.value) return []
  return (currentDialer.value.Agents || []).map(agent => {
...
.rp-filter-dialer { min-width: 200px; }
.rp-filter-label {
